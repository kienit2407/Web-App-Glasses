...
const VOYAGE_EMBEDDING_MODEL =
    process.env.VOYAGE_EMBEDDING_MODEL || "voyage-large-2-instruct";
...
export async function chatWithLlama(
    ...
    const answer =
        completion.choices[0]?.message?.content?.trim() || "Xin lỗi, mình chưa trả lời được.";
    return answer;
}
...
export async function embedTexts(texts: string[]): Promise<number[][]> {
    ...
    const vectors: number[][] = res.data.data.map(
