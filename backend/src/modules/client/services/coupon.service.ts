// src/modules/client/services/coupon.service.ts
...
export const couponService = {
    ...
    computeDiscount(subtotal: number, coupon: any): number {
        ...
        if (coupon.type === "percent") {
            ...
            if (coupon.max_discount) discount = Math.min(raw, coupon.max_discount);
            else discount = raw;
    ...
    async claimCoupon(userId: Types.ObjectId, code: string) {
    ...
    async checkCoupon(userId: Types.ObjectId | null, rawCode: string, subtotal?: number) {
        ...
        if (!coupon) {
            throw new BadRequestException("Coupon không tồn tại hoặc đã ngừng hoạt động");
        }
        ...
        if (
            ...
            throw new BadRequestException(
                `Đơn hàng phải lớn hơn ${coupon.min_order} VND để sử dụng voucher này`
            );
        }
        ...
        if (userId && coupon.per_user_limit != null && coupon.per_user_limit > 0) {
            ...
            if (usedByUser >= coupon.per_user_limit) {
                throw new BadRequestException("Bạn đã dùng hết số lần cho phép của coupon này");
            }
        }
    ...
    async listMyCoupons(userId: Types.ObjectId, subtotal?: number) {
        ...
        const items = rows
            .map((row: any) => {
                ...
                let missing_amount: number | undefined;
