...
import { searchFaceAdvice } from "./faceAdvice.service";
...
export async function classifyIntentWithLLM(
  message: string,
  recentHistory: string // Truyền thêm 1-2 câu lịch sử để hiểu ngữ cảnh
): Promise<ChatIntent> {
  const systemPrompt = `
...
-> Intent là:`;
  ...
  const response = await chatWithLlama([
  ...
  const validIntents: ChatIntent[] = ["order", "face_advice", "product", "smalltalk"];
  if (validIntents.includes(rawIntent as ChatIntent)) {
    return rawIntent as ChatIntent;
  }
...
export async function detectIntentHybrid(
  ...
  if (
    lower.includes("mặt tròn") || lower.includes("mặt vuông") ||
...
function detectGenderPref(message: string): GenderPref {
  const lower = message.toLowerCase();

  // một số pattern cơ bản, bạn có thể bổ sung tuỳ gu
  if (
    lower.includes("kính mát nam") ||
...
==> Hãy viết lại thành MỘT câu hỏi độc lập:`,
  ...
  const rewritten = await chatWithLlama([system, user]);
  console.log("[chat] condensed question:", rewritten);
  return rewritten.trim();
}

/* ================== 3. HELPER CHUNG ================== */
type GenderPref = "male" | "female" | "kids" | null;
...
function isGenericAvailabilityQuestion(message: string): boolean {
  const lower = message.toLowerCase();
  return (
    lower.includes("còn hàng không") ||
...
export function detectIntent(
  ...
  if (product || avail) return "product"; 
...
/* ================== 5. PRODUCT ANSWER (KHÔNG DÙNG LLM) ================== */
...
async function handleFaceAdvice(
  message: string,
  history: ChatHistoryTurn[]
): Promise<string> {
  ...
  const { docs, topScore } = await searchFaceAdvice(searchPixel, 3);
...
`;
  ...
  return await chatWithLlama(messages);
}
...
async function handleProductSearch(
  ...
  if (kind) {
    const res = await productService.getContextRecommendations({
      keywordContext: kind,
  ...
  if (items.length === 0) {
    ...
    const res = await productService.getBudgetRecommendations({
...
`;
  ...
  const productContext = finalItems.map(p =>
    `- Tên: ${p.product_name} | Giá: ${p.price.toLocaleString("vi-VN")}đ | Link: /${p.slug}`
  ).join("\n");
