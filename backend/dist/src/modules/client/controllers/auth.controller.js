...
const format_ip_1 = require("../../../utils/format_ip");
const signUp = (0, try_catch_1.TryCatch)(async (req, res, next) => {
    ...
    const rawIp = req.headers["x-forwarded-for"]?.split(",")[0]?.trim() ||
        ...
        null;
    const ip = (0, format_ip_1.normalizeIp)(rawIp);
    const result = await auth_service_1.authService.signUp(payload, platform, res, { userAgent, ip });
...
const signIn = (0, try_catch_1.TryCatch)(async (req, res, next) => {
    ...
    const result = await auth_service_1.authService.signIn(payload, platform, res, { userAgent, ip });
